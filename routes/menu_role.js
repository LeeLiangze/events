...
router.get('/get_menu', async (req, res, next) => {
    ...
    try {
        ...
        var sql = "select a.menu_id from menu a inner join menu_role b on a.menu_id=b.menu_id where b.role_id=? and a.is_del=0";
        // 所有菜单权限
        var sql2 = "select * from menu where is_del=0 order by parent_id asc , menu_id asc";
...
router.post('/setMenu', async (req, res, next) => {
    ...
    if (e_id && e_id != "" && e_id != 0) {
        ...
        try {
            ...
            var sql = "delete from menu_role where role_id = ?";
            var sql2 = "insert into menu_role(role_id, menu_id) values (?,?)";
