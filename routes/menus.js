...
router.get('/load', async (req, res, next) => {
    ...
    var sqlcount = "select count(*) count from menu where is_del=0";
    var sql = "select * from menu where is_del=0";
...
router.get('/save', async (req, res, next) => {
    ...
    try {
        ...
        if (result.msg != "") {
            ...
            if(e_menu_flag) {
                sql = "select * from menu where menu_flag=? and is_del=0 and type = 0";
            ...
            var ret, sql;
            if (e_id && e_id != 0) {
                sql = "update menu set menu_name=?,parent_id=?,menu_url=?,menu_icon=?, menu_flag=?, type=?, modified_id=?, modified_at=? where menu_id=?";
                ...
                sql = "select * from menu where menu_name=? and is_del=0";
                var menus = await mysql.query(sql, e_menu_name);
                if (menus.length > 0) {
                    result.error = 1;
                    result.msg = "菜单名已经存在！";
                } else {
                    sql = "insert menu(menu_name, parent_id,menu_url,menu_icon,menu_flag,type,creator_id) values (?,?,?,?,?,?,?)";
...
router.delete('/delete', async (req, res, next) => {
    ...
    if (ids && ids.trim() != "") {
        ...
        try {
            ids = ids.split(",");
            var pSql = "select * from menu where parent_id in (";
            var sql = 'delete from menu_role where menu_id in (';
            var sql3 = 'update menu set is_del=1, modified_at=?, modified_id=? where menu_id in (';
...
router.get('/detail', async (req, res, next) => {
    ...
    try {
        var sql = "select * from menu where is_del=0 and menu_id=?";
