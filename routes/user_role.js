...
router.get('/load', async(req, res, next) => {
    ...
    for (var i in result) {
        var obj = result[i];
        var user_id = obj['id'];
        sql = "select b.user_id,a.user_name,a.name,b.role_id,c.role_name,c.description from user a left join user_role b on a.id=b.user_id left join role c on b.role_id=c.role_id where a.id=? and a.is_del=0 and c.is_del=0";
...
router.get('/getRole', async(req, res, next) => {
    ...
    try {
        var sql = "select role_id,role_name from role where is_del=0";
