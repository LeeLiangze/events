...
router.get('/load', async(req, res, next) => {
    var sqlcount = "select count(*) count from role where is_del=0";
    var sql = "select * from role where is_del=0";
...
router.get('/save', async(req, res, next) => {
    ...
    try {
        ...
        if (result.msg != "") {
            ...
            var ret, sql;
            if (e_id) {
                sql = "update role set role_name=?,description=?, modified_id=?, modified_at=? where role_id=?";
                var params = [e_role_name, e_description, user.id, new Date(), e_id];
                ret = await mysql.query(sql, params);
            } else {
                sql = "select * from role where role_name=? and is_del=0";
                var users = await mysql.query(sql, e_role_name);
                if (users.length > 0) {
                    result.error = 1;
                    result.msg = "Role name exists";
                } else {
                    sql = "insert role(role_name, description,creator_id) values (?,?,?)";
...
router.delete('/delete', async(req, res, next) => {
    ...
    try {
        ...
        if (ids && ids.trim() != "") {
            ids = ids.split(",");
            var sql = 'delete from menu_role where role_id in (';
            var sql2 = 'delete from user_role where role_id in (';
            var sql3 = 'update role set is_del=1, modified_at=?, modified_id=? where role_id in (';
